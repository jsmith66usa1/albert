...
try {
  ...
  addLog({ 
    ...
    duration: 0, 
    status: 'ERROR', 
    message: `Offline mode active. Using local neural paths.` 
  });
}
 ...
 * Explicitly logs whether the data came from GLOBAL or LOCAL cache.
 */
async function getFromCache(category: string, key: string, dataType: string): Promise<any> {
  const start = performance.now();
  
  // 1. Try Global Firebase Cache (Shared with all users)
  if (db) {
    try {
      ...
      if (snapshot.exists()) {
        ...
        addLog({ 
          ...
          status: 'CACHE_HIT', 
          message: `Retrieved ${dataType} from shared laboratory memory (Firebase Global).`
        });
  ...
  try {
    ...
    if (local) {
      addLog({ 
        ...
        duration: performance.now() - start, 
        status: 'CACHE_HIT', 
        message: `Retrieved ${dataType} from local browser node (LocalStorage).`
      });
      return local;
    }
  } catch (e) {}
...
async function saveToCache(category: string, key: string, data: string): Promise<void> {
  ...
  // Save to Global Firebase (Shared with all users)
  if (db) {
    try {
      const dbRef = ref(db, `world_brain_v11/${category}/${key}`);
...
6. GOAL: Keep responses relatively concise but overflowing with wonder.`;
  ...
  try {
    const cached = await getFromCache('responses', key, 'TEXT_RESPONSE');
  ...
  try {
    ...
    if (textResult) {
      ...
      addLog({ type: 'AI_TEXT', label: 'SYNTHESIS SUCCESS', duration: performance.now() - start, status: 'SUCCESS', message: 'New response synthesized by the World Brain.' });
      return textResult;
    }
...
export async function generateChalkboardImage(prompt: string): Promise<string> {
  ...
  try {
    const cached = await getFromCache('images', key, 'CHALKBOARD_IMAGE');
    if (cached) return cached;
  } catch(e) {}
  ...
  try {
    ...
    if (imageData) { 
      ...
      addLog({ type: 'AI_IMAGE', label: 'CHALK SKETCHED', duration: performance.now() - start, status: 'SUCCESS', message: 'New diagram sketched on the global chalkboard.' });
      return imageData; 
    }
...
export async function generateEinsteinSpeech(text: string, retries = 1): Promise<string> {
  ...
  try {
    const cached = await getFromCache('audio', key, 'SPEECH_AUDIO');
    if (cached) return cached;
  } catch(e) {}
    ...
    try {
      ...
      if (base64) {
        ...
        addLog({ 
          ...
          duration: performance.now() - start, 
          status: 'SUCCESS', 
          message: 'Vocal patterns derived and broadcasted globally.' 
        });
        return base64;
      }
